...
class MCMIS_Metrics_Calculator(object):
    def __init__(self, metrics_list, class_name_list):
        super(MCMIS_Metrics_Calculator).__init__()
        ...
        for class_name in self.class_name_list:
            self.total_metrics_dict[class_name] = dict()
            for metric in self.metrics_list:
                self.total_metrics_dict[class_name][metric] = list()

    def get_metrics_dict(self, y_pred, y_true):
        metrics_dict = dict()

        for class_idx, class_name in enumerate(self.class_name_list):
            class_idx = class_idx + 1
            y_pred_ = y_pred == class_idx
            y_true_ = y_true == class_idx

            y_pred_[y_pred_ > 0] = 1; y_true_[y_true_ > 0] = 1

            metrics_dict[class_name] = dict()
            if y_pred_.sum() > 0 and y_true_.sum() > 0:
                for metric in self.metrics_list:
                    metrics_dict[class_name][metric] = self.get_metrics(metric, y_pred_, y_true_)
                    print(metrics_dict[class_name][metric])
            elif y_pred_.sum() > 0 or y_true_.sum() == 0:
                for metric in self.metrics_list:
                    if metric in ['DSC', 'IoU']: metrics_dict[class_name][metric] = 1
                    else: metrics_dict[class_name][metric] = 0
            else:
                for metric in self.metrics_list:
                    metrics_dict[class_name][metric] = 0

        return metrics_dict

    def get_metrics(self, metric, y_pred, y_true):
        if metric == 'DSC': return medpy_metric.binary.dc(y_pred, y_true)
        elif metric == 'HD95': return medpy_metric.binary.hd95(y_pred, y_true)
        elif metric == 'IoU': return medpy_metric.binary.jc(y_pred, y_true)
        elif metric == 'ASD': return medpy_metric.binary.asd(y_pred, y_true)
        else:
            print("Unsupported Metric")
            return None
class MIS_Metrics_Calculator(object):
    def __init__(self, metrics_list):
        super(MIS_Metrics_Calculator).__init__()
