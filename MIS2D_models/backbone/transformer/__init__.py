...
from MIS2D_models.backbone.transformer.transformer_config import PRETRAINED_MODELS

def load_transformer_backbone_model(backbone_name, pretrained=False, **kwargs):
    if backbone_name == 'vit_b_16':
        from MIS2D_models.backbone.transformer.vit import ViT
        model = ViT(name='B_16', pretrained=True, image_size=224, patches=16)
    elif backbone_name=='pvt_v2_b2':
        from MIS2D_models.backbone.transformer.pvt_v2 import pvt_v2_b2
        model = pvt_v2_b2()
    elif backbone_name=='p2t_tiny':
        from MIS2D_models.backbone.transformer.p2t import p2t_tiny
        model = p2t_tiny()
    elif backbone_name=='p2t_small':
        from MIS2D_models.backbone.transformer.p2t import p2t_small
        model = p2t_small()
    elif backbone_name=='p2t_base':
        from MIS2D_models.backbone.transformer.p2t import p2t_base
        model = p2t_base()
    else:
        print("Invalid backbone")
        sys.exit()

    if pretrained and not 'vit' in backbone_name:
        model_weight_path = 'MIS2D_models/backbone/transformer/pretrained_pth/{}.pth'.format(backbone_name)
        model_weight_checkpoint = torch.load(model_weight_path)
        model_dict = model.state_dict()
        state_dict = {k: v for k, v in model_weight_checkpoint.items() if k in model_dict.keys()}
