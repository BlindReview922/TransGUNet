...
class ECA(nn.Module):
    ...
    def forward(self, x):
        ...
        channel_attention_map = y_avg + y_max
        channel_attention_map = torch.sigmoid(channel_attention_map)
...
class GraphConv2d(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, conv='edge', act='relu', norm=None, bias=True, kernel_size=1,
                 padding=0, groups=4):
        super(GraphConv2d, self).__init__()
        if conv == 'edge':
            self.gconv = EdgeConv2d(in_channels, out_channels, act, norm, bias)
        elif conv == 'mr':
            self.gconv = MRConv2d(in_channels, out_channels, act, norm, bias, kernel_size=kernel_size, padding=padding,
                                  groups=groups)
        elif conv == 'sage':
...
class DyGraphConv2d(GraphConv2d):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=9, dilation=1, conv='edge', act='relu',
        ...
        super(DyGraphConv2d, self).__init__(in_channels, out_channels, conv, act, norm, bias, kernel_size=kernel_size,
    ...
    def forward(self, x, relative_pos=None):
        ...
        x = x.reshape(B, C, -1, 1).contiguous() # [B, 256, 28, 28] -> [B, 256, 784, 1]
        edge_index = self.dilated_knn_graph(x, y, relative_pos)
        x = super(DyGraphConv2d, self).forward(x, edge_index, y) # [B, 256, 784, 1]
...
class Grapher(nn.Module):
    ...
    def __init__(self, in_channels, kernel_size=9, dilation=1, conv='edge', act='relu', norm=None,
        ...
        self.graph_conv = DyGraphConv2d(in_channels, in_channels * 2, kernel_size, dilation, conv,
                                        ...
                                        groups=in_channels)  # in_channels * 2
